...
from juniorguru.sync.employments.pipelines.identifier import Pipeline, MissingIdentifyingField
...
def test_identifier_keeps_id_set_by_spider_even_if_url_missing(item, spider):
    item['id'] = '44df50581c3a0c67657c40a587c22a19e10d414b593fcfaea5b01ae9'
    del item['url']
    item = Pipeline().process_item(item, spider)
...
def test_identifier_sets_default_id(item, spider):
    del item['id']
    item['url'] = 'https://example.com/we-are-hiring/123'
    item = Pipeline().process_item(item, spider)
...
def test_identifier_drops(item, spider):
    ...
    with pytest.raises(MissingIdentifyingField, match='url'):
