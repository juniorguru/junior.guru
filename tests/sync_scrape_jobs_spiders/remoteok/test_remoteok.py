...
def test_spider_parse_job():
    ...
    assert job["company_logo_urls"] == [
        "https://remoteok.io/assets/jobs/af6ada420dbb093b6fc3cc1dd1a75bb41603782017.png"
    ]
    assert job["source"] == "remoteok"
    assert job["source_urls"] == ["https://example.com/example/"]
    assert normalize_html(job["description_html"]) == normalize_html(
        Path(FIXTURES_DIR / "description.html").read_text()
    )
