...
def test_spider_parse():
    ...
    assert (
        job_requests[0].url
        == "https://www.linkedin.com/jobs-guest/jobs/api/jobPosting/1846698040"
    )
...
def test_spider_parse_job():
    ...
    jobs = list(
        linkedin.Spider().parse_job(response, "https://example.com/search?foo=1")
    )
    ...
    assert (
        job["url"]
        == "https://cz.linkedin.com/jobs/view/junior-bi-developer-bi4sg-at-komer%C4%8Dn%C3%AD-banka-2701029809"
    )
...
def test_spider_parse_job_apply_on_company_website():
    ...
    assert (
        job["apply_url"]
        == "https://jobs.siemens.com/jobs/240215?lang=en-us&jobPipeline=juniorguru%3FsourceType%3DPREMIUM_POST_SITE&source=juniorguru%28Wrap%29"
    )
...
def test_spider_verify_job_apply_on_company_website_with_proxies_and_validations():
    ...
    assert (
        job["url"]
        == "https://cz.linkedin.com/jobs/view/junior-software-engineer-package-maintainer-part-time-brno-southeast-red-hat-2689458333"
    )
    assert (
        job["apply_url"]
...
def test_clean_proxied_url():
    ...
    assert (
        linkedin.clean_proxied_url(url)
...
def test_clean_validated_url():
    ...
    assert (
        linkedin.clean_validated_url(url)
        == "http://red-hat-1.talentify.io/job/junior-software-engineer-package-maintainer-part-time-brno-southeast-red-hat-89305"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "JS Frontend Developer (Typescript, Angular) – office nebo 100% remotely",
            True,
        ),
    ],
)
