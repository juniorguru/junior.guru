...
StubChannel = namedtuple(
    "StubChannel",
    "name, type",
    defaults=["sample-channel-name", discord.ChannelType.text],
)

StubCategory = namedtuple(
    "StubCategory",
    "name, type, channels, position",
...
@pytest.mark.parametrize(
    "existing_categories_count, max_channels_needed, expected",
    [
        (0, 0, 0),
        (0, 10, 1),
        (0, 42, 1),
...
def test_calc_missing_categories_count(
    existing_categories_count, max_channels_needed, expected
):
    assert (
        calc_missing_categories_count(existing_categories_count, max_channels_needed)
        == expected
    )
...
def test_get_available_category_sorts_by_position():
    categories = [
        StubCategory(channels=create_channels(1), position=50),
