...
@pytest.mark.parametrize(
    "today, last_run_on, expected",
    [
        pytest.param(date(2022, 3, 28), None, True, id="on Monday, for the first time"),
        pytest.param(
            date(2022, 3, 30), None, False, id="on random day, for the first time"
        ),
        pytest.param(
            date(2022, 3, 30),
            date(2022, 3, 28),
            False,
            id="on random day, last run this Monday",
        ),
        pytest.param(
            date(2022, 4, 3),
            date(2022, 3, 28),
        ...
        pytest.param(
            ...
            date(2022, 3, 21),
            True,
            id="on Monday, last run on previous Monday",
        ),
        pytest.param(
            date(2022, 3, 30),
            date(2022, 3, 21),
            True,
            id="on random day, last run on previous Monday",
        ),
        pytest.param(
            date(2022, 3, 28),
            date(2022, 3, 23),
            True,
            id="on Monday, last run on previous random day",
        ),
        pytest.param(
            date(2022, 3, 30),
            date(2022, 3, 23),
            True,
            id="on random day, last run on previous random day",
        ),
        pytest.param(
            date(2022, 3, 28),
            date(2022, 3, 28),
            False,
            id="on Monday, already sent today",
        ),
    ],
)
...
def test_debug_message():
    message = dict(
        from_email=("From", "sender@example.com"),
        to_emails=[("To 1", "to1@example.com"), ("To 2", "to2@example.com")],
        bcc_emails=[("Bcc 1", "bcc1@example.com"), ("Bcc 2", "bcc2@example.com")],
        subject="Subject",
    ...
    assert message == dict(
        from_email=("From", "sender@example.com"),
        to_emails=[("To 1", "honza@junior.guru"), ("To 2", "honza@junior.guru")],
        bcc_emails=[],
