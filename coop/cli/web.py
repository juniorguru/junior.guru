...
from coop.web_legacy.__main__ import main as flask_freeze
...
def build_static(output_path: Path):
    ...
    shutil.copytree(
        "coop/images/screenshots-overrides",
...
def serve(context, output_path: Path, open: bool):
    ...
    server.watch("coop/**/*.js", rebuild_static)
    server.watch("coop/**/*.scss", rebuild_static)
    server.watch("coop/images/", rebuild_static)
    server.watch("coop/data/", rebuild_flask_and_mkdocs, ignore=ignore_data)
    server.watch("coop/lib/", rebuild_flask_and_mkdocs)
    server.watch("coop/models/", rebuild_flask_and_mkdocs)
    server.watch("coop/web/", rebuild_mkdocs)
    server.watch("coop/web_legacy/", rebuild_flask_and_mkdocs)
    server.serve(
        host="localhost",
