...
function freezeFlask() {
  ...
  const proc = spawn('poetry', ['run', 'jg', 'web', 'freeze'], { stdio: 'inherit' });
...
function buildMkDocsFiles() {
  const proc = spawn('poetry', ['run', 'jg', 'web', 'mkdocs'], { stdio: 'inherit' });
  return new Promise((resolve, reject) => {
    proc.on('exit', (code) => { code ? reject(new Error(`Exit code ${code}`)) : resolve(code); });
    proc.on('error', (error) => { reject(error); });
  });
...
async function serveWeb() {
  connect.server({ root: 'public/', port: 5042, livereload: true });
