...
<div class="blockquotes-roll animated">
{{ blockquote_avatar(
...
{% for diary in club_diaries|sample(8) %}
  <li class="diaries-item" title="{{ diary.name }}" aria-label="{{ diary.name }}">{{ 'journal-bookmark-fill'|icon }} {{ diary.name|truncate(50, True, '…') }}</li>
{% endfor %}
</ul>
{% endcall %}
</section>
...
{% endcall %}
<div class="blockquotes-roll standout-top animated">
{{ blockquote_avatar(
