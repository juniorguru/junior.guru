...
class ScrapingProxiesMiddleware:
    ...
    def from_crawler(cls, crawler):
        ...
        return cls(
            proxies_list, user_agents=crawler.settings.getlist("PROXIES_USER_AGENTS")
        )
    ...
    def rotate_proxies(self, request):
        ...
        try:
            ...
            if prev_latency is None:
                ...
                next_latency = request.meta.get(
                    "download_latency", request.meta["download_timeout"]
                )
        ...
        if next_proxy_url:
            logger.debug(
                f"Rotating proxies: {next_proxy_url} instead of {prev_proxy_url}"
            )
            return request.replace(
                headers=self.rotate_user_agent(request.headers),
                meta={"proxy": next_proxy_url, **meta},
                dont_filter=True,
            )
        logger.warning("No proxies, continuing without proxy")
        return request.replace(
            headers=self.rotate_user_agent(request.headers), meta=meta, dont_filter=True
        )
    ...
    def process_request(self, request, spider):
        ...
        if not request.meta.get(
            "proxies", True
        ):  # allows to explicitly turn proxies off for a particular request
            return
    ...
    def process_response(self, request, response, spider):
        ...
        if self.is_invalid_response(response):
            ...
            logger.info(
                f"Invalid response: {response!r} proxied via {proxy_url} ({user_agent})"
            )
            return self.rotate_proxies(request)
        logger.debug(f"Got proxied response {response!r}")
        self.record_proxy_latency(
            proxy_url, request.meta.get("download_latency", 0)
        )  # requests from cache don't have this set
        return response
