...
function updateToC(toc) {
  const position = getBottomPosition(toc.bar) + getScrollMarginTop() + SCROLL_MARGIN_TOP_EXTRA_PX;
...
function getScrollMarginTop() {
  const style = getComputedStyle(document.querySelector(SCROLL_MARGIN_TOP_SELECTOR));
...
function getLastElement(elements) {
  return elements.reduce((latestElement, element) => {
    return getTopPosition(latestElement) > getTopPosition(element) ? latestElement : element;
  })
}
