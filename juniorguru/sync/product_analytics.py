...
def main():
    ...
    time_ranges = [dict(start=month.replace(day=1).isoformat(),
                        end=month.isoformat())
                   for month
                   in charts.generate_months(date_from, date_to)][:MONTHS_RANGE]

    for time_range in time_ranges:
        logger.debug(f'Fetching analytics from {time_range["start"]} to {time_range["end"]}')
