...
async def manage_channels(client: ClubClient):
    ...
    moderators_role = [
        role for role in client.club_guild.roles if role.id == MODERATORS_ROLE
    ][0]
    ...
    members = [
        ...
        if (
            member.id in beta_users_ids
            or member.id in moderators_ids
            or member.first_seen_on() > BETA_USERS_DATE
        )
    ]
...
async def fetch_beta_users(client: ClubClient):
    announcements_channel = await client.club_guild.fetch_channel(
        ClubChannelID.ANNOUNCEMENTS
    )
    beta_users_message = await announcements_channel.fetch_message(BETA_USERS_MESSAGE)
    beta_users_reaction = [
        reaction
        for reaction in beta_users_message.reactions
        if reaction.emoji == BETA_USERS_EMOJI
    ][0]
    return [beta_user.id async for beta_user in beta_users_reaction.users()]
...
async def execute_channels_operations(client: ClubClient, operations):
    channels_operations_chunks = chunks(operations, size=CHANNELS_OPERATIONS_CHUNK_SIZE)
    for n, channels_operations_chunk in enumerate(channels_operations_chunks, start=1):
        logger.debug(
            f"Processing chunk #{n} of {len(channels_operations_chunk)} channels operations"
        )
