...
async def main(client):
    ...
    log.info('Saving votes from channel history')
    for message in ClubMessage.channel_listing(JOBS_CHANNEL):
        ...
        try:
            ...
            log.info(f'Found {job_url}, has score {employment.juniority_votes_score} ({employment.juniority_votes_count} votes)')
            ...
            if employment.juniority_votes_score != votes_score or employment.juniority_votes_count != votes_count:
                log.info(f'Updating {job_url}, new score {votes_score} ({votes_count} votes)')
                employment.juniority_votes_score = votes_score
                employment.juniority_votes_count = votes_count
                with db:
                    employment.save()
        except Employment.DoesNotExist:
            log.info(f'Not found {job_url}')

    log.info('Posting new jobs to the channel')
    for job in Job.listing():
        if job.effective_link in seen_urls:
            continue

        if DISCORD_MUTATIONS_ENABLED:
            log.info(f'Posting {job.effective_link}')
