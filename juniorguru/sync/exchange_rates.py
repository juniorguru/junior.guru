...
@click.option(
    "--path",
    "yaml_path",
    default="juniorguru/data/exhange_rates.yml",
...
def main(yaml_path: Path):
    ...
    try:
        ...
        exchange_rates = [
            parse_exchange_rate(line) for line in parse_lines(response.text)
        ]
        exchange_rates = [
            exchange_rate
            for exchange_rate in exchange_rates
            if exchange_rate["code"] in CURRENCIES
        ]
        exchange_rates = sorted(exchange_rates, key=itemgetter("code"))
        yaml_path.write_text(
            strictyaml.as_document(exchange_rates, YAML_SCHEMA).as_yaml()
        )
