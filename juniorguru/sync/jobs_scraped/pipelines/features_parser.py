...
def rule(identifier, *patterns, ignorecase=True, any_order=False):
    ...
    pattern_lists = [
        ([pattern] if isinstance(pattern, str) else list(pattern))
...
RULES_EN = rules(
    [
        rule(
            ...
            TECH_DEGREES_EN + [r"degree", r"studies"],
            TECH_DEGREE_FIELDS_EN,
        ),
        ...
        rule(
            "ADVANCED_REQUIRED",
            r"(proficien(cy|t)|fluen(cy|t)|well.?versed) (in|with|\w+ing)",
        ),
        ...
        rule(
            "ADVANCED_REQUIRED",
            r"you",
            r"(consider yourself|are)",
            r"(expert|researcher|hacker)",
        ),
        ...
        rule(
            ...
            r"(execute|work(ing)?|operat(e|ing)|solv(e|ing))",
            r"(independen\w+|autonomou\w+)",
        ),
        ...
        rule(
            "EXPLICITLY_SENIOR",
            [r"seasoned", r"experienced", r"practiced", r"specialist"],
            [r"engineer", r"developer"],
        ),
        rule(
            "EXPLICITLY_JUNIOR",
            [r"junior", r"padawan", r"internship", r"intern", r"trainee"],
        ),
...
SUPPRESSING_RULES_EN = rules(
    [
        ...
        rule(
            "ADVANCED_REQUIRED",
            r"(professional|personal|technical) (requirements|skills|expertise)",
        ),
        ...
        rule(
            "EXPLICITLY_SENIOR",
            [r"collaborate", r"work"],
            r"with",
            r"(engineer|developer)s?",
        ),
...
RULES_CS = rules(
    [
        ...
        rule(
            "TECH_DEGREE_REQUIRED",
            TECH_DEGREES_CS,
            TECH_DEGREE_FIELDS_CS,
            any_order=True,
        ),
        rule("ADVANCED_REQUIRED", ADVANCED_SKILLS_ADJECTIVES_CS, SKILLS_NOUNS_CS),
        rule(
            "YEARS_EXPERIENCE_REQUIRED",
            r"(a(le)?spoň|minim\w+|min|\d+)[\W]*(rok|let|rok[yůu]|roční|letou)",
        ),
        ...
        rule(
            "EXPLICITLY_JUNIOR",
            [r"junior\w*", r"jnr", r"padawan\w*", r"učeň", r"internship", r"stáž\w*"],
        ),
        ...
        rule(
            "JUNIOR_FRIENDLY",
            r"(prostor\w*|příležitost\w*|možnost\w*)",
            r"získ\w+",
            r"(zkušen\w+|prax\w+)",
        ),
        ...
        rule(
            "JUNIOR_FRIENDLY",
            [r"vhodn\w+", r"nebráníme"],
            [r"student\w*", r"absolvent\w*"],
        ),
        ...
        rule(
            ...
            r"mentor\w+",
            [r"ze strany", r"od"],
            [r"kolegů", r"zkušen\w+", r"senior\w+"],
        ),
        ...
        rule(
            "LEARNING_REQUIRED",
            r"(chce\w*|chtěj\w*|chtít|chuť|ochot\w+|schopn\w+)",
            r"(učit|na sobě|rozvíj\w+)",
        ),
...
SUPPRESSING_RULES_CS = rules(
    [
        ...
        rule(
            "EXPLICITLY_SENIOR",
            [r"společně", r"spolupr[aá]c\w+", r"tým\w*", r"tandem\w*"],
            r"se?",
            r"senior\w*",
        ),
...
async def process(item: dict, cache: Cache | None = None) -> dict:
    ...
    item["features"] = [
        dict(name=LANG_MAPPING[item["lang"]], origin="language_filter")
    ] + [
        dict(
            name=rule_id, origin="features_parser", sentence=sentence, patterns=patterns
        )
...
def deduplicate(parse_results):
    ...
    return [
        (rule_id, sentence, patterns) for (rule_id, sentence), patterns in seen.items()
    ]


def parse_from_sentences(sentences, lang):
    return itertools.chain.from_iterable(
        parse_from_sentence(sentence, lang) for sentence in sentences
    )
...
def parse_from_sentence(sentence, lang):
    for rule_id, rule_re in RULES[lang]:
        ...
        if match and not is_supressed(rule_id, sentence, lang):
...
def is_supressed(rule_id, sentence, lang):
    for suppressing_rule_id, suppressing_rule_re in SUPPRESSING_RULES[lang]:
        is_relevant = suppressing_rule_id == "" or suppressing_rule_id == rule_id
