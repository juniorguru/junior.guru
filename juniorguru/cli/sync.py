...
class ChainGroup(click.Group):
    command_class = ChainCommand
    ...
    def list_commands(self, *args, **kwargs):
        if not self.commands:
            self.commands = dict(self.load_dynamic_commands())
        return super().list_commands(*args, **kwargs)


@click.group(cls=ChainGroup, chain=True)
@click.option('--id', envvar='CIRCLE_BUILD_NUM', default=perf_counter_ns)
@click.pass_context
def subcommands(context, id):
...
@click.pass_context
def all(context):
    for name, command in subcommands.commands.items():
...
def ci(context, job, node_index, nodes):
    names_commands_map = subcommands.commands
    names_dependencies_map = {name: command.requires for name, command in names_commands_map.items()}

    if job == 'sync-1':
        exclude = {name for name, command in names_commands_map.items() if command.requires}
        chains = get_parallel_chains(names_dependencies_map, exclude=exclude)
    elif job == 'sync-2':
        exclude = {name for name, command in names_commands_map.items() if not command.requires}
        chains = get_parallel_chains(names_dependencies_map, exclude=exclude)
    else:
    ...
    for name in chains[node_index]:
        command = subcommands.get_command(context, name)
        context.invoke(command)


main = click.CommandCollection(sources=[click.Group(commands=[all, ci]), subcommands])
